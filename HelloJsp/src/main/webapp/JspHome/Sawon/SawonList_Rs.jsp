...
<head>
    ...
    <title>사원검색 (ResultSet) Page</title>
	<%----------------------------------------------------------------------
	...
	--------------------------------------------------------------------------%>
	<script type="text/javascript" src="JS/SawonList.js">
		// -----------------------------------------------------------------
...
<%
	...
	// ---------------------------------------------------------------------
	String sAge = null;								// 나이
	String sGender = null;							// 성별
	String sDept = null;							// 부서
	// ---------------------------------------------------------------------
	...
	sAge = (sAge.length() > 0) ? sAge : "0";		// 나이 길이 확인(0:전체)
...
<body class="Body">
	...
	<form name="form1" action="" method="post">
		...
		<div id="divModalParent">
			...
			----------------------------------------------------------------------%>
			<hr class="Line">
			<p  class="Title">사원검색 (ResultSet)</p>
			<hr class="Line">
			<%------------------------------------------------------------------
				사원검색 조건
			----------------------------------------------------------------------%>
			...
			<table class="Table">
					...
					if (bContinue == true)
					{
						while(this.sawonDAO.DBMgr.Rs.next() == true)
						{
							...
							sSawonDeleteCall = String.format("SawonDelete(%d,'%s','%s','%s','%s','%s','%s')",
															  this.sawonDAO.DBMgr.Rs.getInt("Sabun"),
															  this.sawonDAO.DBMgr.Rs.getString("Name"),
															  sAge, sGender, sDept,
															  this.sawonDAO.DBMgr.Rs.getString("DeptName"),
															  this.sawonDAO.DBMgr.Rs.getString("StCdName"));
				%>
				<tr class='TableRow'>
				<%					
				...
				%>
					<td><a     class="UpdateLink" id="btnUpdate<%=nRowCount %>" href="#" onclick="UpdateSetColor('btnUpdate<%=nRowCount %>'); ShowModalWindow('ModalPopup.jsp');">수정</a></td>
